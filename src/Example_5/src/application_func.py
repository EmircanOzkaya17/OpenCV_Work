...
class ImageProcessor:
    def __init__(self, image_path, threshold1=None, threshold2=None, min_area=None, max_area=None):
        ...
        # Default parameters (if no parameters are passed)
        self.threshold1 = threshold1 if threshold1 is not None else 100
        self.threshold2 = threshold2 if threshold2 is not None else 200
        self.min_area = min_area if min_area is not None else 500
        self.max_area = max_area if max_area is not None else 50000
        
        self.load_image()
    
    def load_image(self):
        """Upload the image and complete the preliminary processes."""
        if not os.path.exists(self.image_path):
            raise ValueError(f"No image found: {self.image_path}")
        
        self.original = cv2.imread(self.image_path)
        if self.original is None:
            raise ValueError(f"Image could not be loaded: {self.image_path}")
    ...
    def add_info_text(self, fruit_count):
        """Print the parameter information to the screen."""
    ...
    def save_results(self, output_path=None):
        ...
        edge_output = f"edge_{os.path.basename(output_path)}"
        cv2.imwrite(edge_output, self.edges)
        print(f"Results recorded: {output_path} ve {edge_output}")